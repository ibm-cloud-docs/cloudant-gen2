...
{{site.data.keyword.cloudantfull}}'s [search indexes](/docs/cloudant-gen2?topic=cloudant-gen2-cloudant-search) and [MapReduce views](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce)
...
a body,
and a timestamp.
You create a [MapReduce view](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce) to sort your documents by timestamp.
...
-   The construction of an index happens asynchronously.
    ...
    you must poll {{site.data.keyword.cloudant_short_notm}}'s [`_active_tasks`](/docs/cloudant-gen2?topic=cloudant-gen2-active-tasks#active-tasks) endpoint.
...
The `stale` option is still available, but the more useful options `stable` and `update` are available and must be used instead. For more information, see [Accessing a stale view](/docs/cloudant-gen2?topic=cloudant-gen2-using-views#view-freshness).
{: note}
