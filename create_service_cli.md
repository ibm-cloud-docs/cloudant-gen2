...
The `--parameters` JSON is of the following form:
...
3. To see a list of resource groups, run the following command.
    ...
    **Comment from Karen**
    Need a standard plan replacement for step 4
    {: note}

4. Create an instance of an {{site.data.keyword.cloudant_short_notm}} service by using the `standard-gen2` plan.
    The instance name is `cs20170517a` in the `eu-de` location.

    ```sh
    ibmcloud resource service-instance-create cs20170517a cloudantnosqldb standard-gen2 eu-de
    ```
    {: codeblock}
...
5. After you create the service instance, see the following example message.
    ...
    cs20170517a   eu-de      active       service_instance
    ```
    {: pre}
...
For more information about the fields included in the service credentials, see the [IAM guide](/docs/cloudant-gen2?topic=cloudant-gen2-managing-access-for-cloudant).
...
2. After you receive the request to create credentials for the service instance, review the response from {{site.data.keyword.cloud_notm}} that contains a message similar to the one in the following example.
    ...
    Credentials:
                   ...
                   iam_apikey_description:   Auto generated apikey during resource-k  ey operation for Instance - crn:v1:bluemix:public:cloudantnosqldb:us-south:a/
                   iam_apikey_id:            575299285b283af214
...
2. After you receive the request to retrieve the credentials for the service instance, review the response from {{site.data.keyword.cloud_notm}} that contains a message similar to the one in the following (abbreviated) example.
    ...
    Credentials:
                   apikey:                   XXXXX-XXXXXX_XXXXXXXXXXXXX-XXXXXXXXXXX
                   iam_apikey_description:   Auto generated apikey during resource-key operation for Instance - crn:v1:bluemix:public:cloudantnosqldb:us-south:a/
                   iam_apikey_id:            575299285b283af214
                   iam_apikey_name:          auto-generated-apikey-621ffde2-ea10-4318-b297-d6d849cec48a
                   iam_role_crn:             crn:v1:bluemix:public:iam::::serviceRole:Manager
                   iam_serviceid_crn:        crn:v1:bluemix:public:iam-identity::a/b42116849bb7e2abb0841ca25d28ee4c::serviceid:ServiceId-53f9e2a2-cdfb-4f90-b072-bfffafb68b3e
                   url:                      https://00000000-0000-0000-0000-00000000.abc.us-east.cloudant.dataservices.appdomain.cloud
                   vpe_url:                  https://00000000-0000-0000-0000-00000000.private.abc.us-east.cloudant.dataservices.appdomain.cloud
    ```
    {: pre}
