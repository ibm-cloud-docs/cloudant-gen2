...
Sending a `GET` request to `https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/_changes`
...
| `filter`       | Name of [filter function](/docs/cloudant-gen2?topic=cloudant-gen2-design-documents#filter-functions) to use to get updates. The filter is defined in a [design document](/docs/cloudant-gen2?topic=cloudant-gen2-design-documents#design-documents). | `string` | No filter. |
...
is explained in the
[Replication guide](/docs/cloudant-gen2?topic=cloudant-gen2-replication-guide#how-does-replication-affect-the-list-of-changes-).
...
[filter function](/docs/cloudant-gen2?topic=cloudant-gen2-design-documents#predefined-filter-functions) to apply to the changes feed.
Additionally,
...
`_selector`
:   Returns changes for documents that match the `selector` request body parameter. The [selector syntax](/docs/apis/cloudant/cloudant-gen2#postfind){: external} is the same as the syntax that is used for [`_find`](/docs/apis/cloudant/cloudant-gen2#postexplain){: external}. If you want to use a selector filter, you must use the `POST` changes feed (as you can't supply a document body with a GET request). Use the `_selector` method of filtering instead of the `_view` filtering method because it's faster and easier to use.

	For more information, see the [API documentation](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2#getchanges-changes-request).

`_view`
:   Enables use of an existing [map function](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce#a-simple-view) as the filter.
...
All Go examples require the `service` object to be initialized. For more information, see the API documentation's [Authentication section](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2?code=go#authentication-with-external-configuration) for examples.
{: go}
...
Use the `since` parameter like a [bookmark](/docs/apis/cloudant/cloudant-gen2#bookmark-pagination) to paginate the changes feed.
For specific details and examples see the API documentation topic [Paging the changes feed](/docs/apis/cloudant/cloudant-gen2#paging-the-changes-feed).
