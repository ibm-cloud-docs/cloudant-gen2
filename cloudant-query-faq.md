...
For more information, see [Creating an Index](/docs/cloudant-gen2?topic=cloudant-gen2-query).
...
The [`POST /{db}/_explain`](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2?code=java#postexplain) API endpoint when passed a JSON object that is usually sent to the [`POST /{db}/_find`](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2#postfind) endpoint, explains how such a query is handled and which indexes, if any, might be used.
...
Any of the [combination operators](/docs/cloudant-gen2?topic=cloudant-gen2-operators) other than `$and` can make a query do a full database scan without the help of a secondary index. For example, if an `$or` operator is used, then no secondary index can be used to assist the query. If in doubt, use the [`POST /{db}/_explain`](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2?code=java#postexplain) endpoint to check that an index is used, and the `execution_stats: true` parameter to measure the efficiency of each query.
...
For more information, see [Retrieving query plans](/docs/cloudant-gen2?topic=cloudant-gen2-explain-plans).
...
For more information, see [Creating a `type=text` index](/docs/cloudant-gen2?topic=cloudant-gen2-text-indexes).
...
For more information, see [Creating a partial index](/docs/cloudant-gen2?topic=cloudant-gen2-partial-indexes).
...
Partitioned indexes can be used when you run only a partitioned query, for example, by using the [`GET /<dbname>/_partition/<partition key>/_find`](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2?code=java#postpartitionfind-partitioned-databases) API endpoint.
...
Yes! {{site.data.keyword.cloudant_short_notm}} Query has a [$regex operator](/docs/cloudant-gen2?topic=cloudant-gen2-operators#the-regex-operator) that allows regular expression terms within a query.
