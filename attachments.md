...
If you include the attachment as an [inline](/docs/cloudant-gen2?topic=cloudant-gen2-how-to-use-attachments#inline) component of the overall JSON, the attachment content is represented by using BASE64 form.
{: note}
...
Attachments aren't permitted on documents in [`_replicator`](/docs/apis/cloudant/cloudant-gen2#postreplicate){: external} or [`_users`](/docs/apis/cloudant/cloudant-gen2#putsecurity){: external} databases.
{: important}

## Create or update
{: #create-update}

To create a new attachment at the same time as creating a new document, include the attachment as an [inline](/docs/cloudant-gen2?topic=cloudant-gen2-how-to-use-attachments#inline) component of the JSON content.

To create a new attachment on an existing document,
or to update an attachment on a document,
make a PUT request with the document's most recent `_rev` to `https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/$DOCUMENT_ID/$ATTACHMENT`.
...
All Go examples require the `service` object to be initialized. For more information, see the API documentation [Authentication section](https://cloud.ibm.com/docs/apis/cloudant/cloudant-gen2?code=go#authentication-with-external-configuration) for examples.
{: go}
...
make a `GET` request to `https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/$DOCUMENT_ID/$ATTACHMENT`.
...
to `https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/$DOCUMENT_ID/$ATTACHMENT`.
If you don't supply the most recent `_rev`,
the response is a [409 error](/docs/apis/cloudant/cloudant-gen2#error-handling){: external}.
