...
and review the [API reference](/docs/apis/cloudant/cloudant-gen2#intro){: external} documentation.
...
and [deleting them](/docs/cloudant-gen2?topic=cloudant-gen2-delete-a-doc) if they're no longer needed.
...
1.  `initializing` - The replication was added to the scheduler, but isn't yet initialized or scheduled to run. The status occurs when a new or updated replication document is stored within the [`_replicator` database](/docs/cloudant-gen2?topic=cloudant-gen2-replication-api#the-replicator-database).
2.  `error` - The replication can't be turned into a job. This error might be caused in several different ways. For example, the replication must be [filtered](/docs/cloudant-gen2?topic=cloudant-gen2-design-documents#filter-functions), but it wasn't possible to fetch the filter code from the source database.
...
6.  `completed` - The replication job completed. This state doesn't apply to [continuous replications](/docs/cloudant-gen2?topic=cloudant-gen2-replication-api#continuous-replication).
...
- [`/_scheduler/docs`](/docs/apis/cloudant/cloudant-gen2#getschedulerdocs){: external}
- [`/_scheduler/jobs`](/docs/apis/cloudant/cloudant-gen2#getschedulerjobs){: external}
...
1.  Create a [replication document](/docs/apis/cloudant/cloudant-gen2#putreplicationdocument){: external} that describes the needed replication, and store the document in the [replicator database](/docs/cloudant-gen2?topic=cloudant-gen2-replication-api#the-replicator-database).
...
Checkpoints for replication are [enabled by default](/docs/apis/cloudant/cloudant-gen2#postreplicate){: external},
...
```json
{
  ...
  "target": {
    "url": "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/db",
    "auth": {
      "basic": {
        "username": "$USERNAME",
        "password": "$PASSWORD"
...
[filtering the `_changes` feed](/docs/cloudant-gen2?topic=cloudant-gen2-design-documents#filter-functions).
{: tip}
...
```json
{
  ...
  "target": {
    "url": "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/example-database",
    "auth": {
      "basic": {
        "username": "$USERNAME",
        "password": "$PASSWORD"
...
The `selector` option provides performance benefits when compared with using the `filter` option. Use the `selector` option whenever possible. For more information, see the [`selector`](/docs/apis/cloudant/cloudant-gen2#postfind){: external} documentation.
...
```json
{
	...
	"target": {
	  "url": "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/bar",
	  "auth": {
	    "basic": {
	      "username": "$USERNAME",
	      "password": "$PASSWORD"
...
see [Performance considerations](/docs/cloudant-gen2?topic=cloudant-gen2-how-to-use-attachments#performance-considerations).
