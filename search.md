...
| `facet` | Creates a faceted index. For more information, see [Faceting](/docs/cloudant-gen2?topic=cloudant-gen2-cloudant-search#faceting). | `true` | `false` | `false` |
| `index` | Whether the data is indexed, and if so, how. If set to `false`, the data can't be used for searches, but can still be retrieved from the index if `store` is set to `true`. For more information, see [Analyzers](/docs/cloudant-gen2?topic=cloudant-gen2-search-analyzers). | `true`, `false` | `true` |
...
Analyzers are settings that define how to recognize terms within text. For more information, see [Search analyzers](/docs/cloudant-gen2?topic=cloudant-gen2-search-analyzers).
...
```http
Host: $ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/_search_analyze" \
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/_search_analyze" -H "Content-Type: application/json"
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/_partition/$PARTITION_KEY/_design/$DDOC/_search/$INDEX_NAME?include_docs=true&query=\"*:*\"&limit=1"
```
{: codeblock}
{: curl}
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/_design/$DDOC/_search/$INDEX_NAME?include_docs=true&query=\"*:*\"&limit=1"
```
{: codeblock}
{: curl}
...
Using `include_docs=true` might have [performance implications](/docs/cloudant-gen2?topic=cloudant-gen2-using-views#multi-document-fetching).
{: important}
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/_design/$DDOC/_search/$INDEX_NAME" -X POST -H "Content-Type: application/json" -d @search.json
```
{: codeblock}
{: curl}
...
Use [bookmark pagination](/docs/apis/cloudant/cloudant-gen2#bookmark-pagination) for search queries.
For specific details and examples see the API documentation topic [Paging on search index queries](/docs/apis/cloudant/cloudant-gen2#paging-on-search-index-queries).
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/examples/_design/cities-designdoc/_search/cities?q=lat:\[0+TO+90\]&sort=\"<distance,lon,lat,-74.0059,40.7127,km>\""
```
{: codeblock}
{: curl}
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/movies/_design/searches/_search/movies?q=\"movie_name:Azazel\"&highlight_fields=\[\"movie_name\"\]&highlight_pre_tag=\" \"&highlight_post_tag=\" \"&highlights_size=30&highlights_number=2" \
	-X GET
```
{: codeblock}
{: curl}
...
```sh
curl "https://$ACCOUNT.abc.cloudant.eu-de.dataservices.appdomain.cloud/$DATABASE/_design/$DDOC/_search_info/$INDEX_NAME" \
     -X GET
```
{: codeblock}
{: curl}
