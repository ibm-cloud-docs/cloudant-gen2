...
or completed?
The [`_active_tasks` endpoint](/docs/cloudant-gen2?topic=cloudant-gen2-active-tasks#active-tasks) provides information about all ongoing tasks.
However,
...
For more information, see the [Using {{site.data.keyword.cloudantfull}}](/docs/cloudant-gen2?topic=cloudant-gen2-ibm-cloudant-basics) for a complete guide to the available options.
...
you might have to [cancel the replication process](/docs/apis/cloudant/cloudant-gen2#deletereplicationdocument){: external} and start it again.
...
Replication uses [checkpoints](/docs/cloudant-gen2?topic=cloudant-gen2-replication-guide#checkpoints), which means that content that is already replicated and unchanged doesn't have to be replicated again if the replication is restarted.
{: note}
