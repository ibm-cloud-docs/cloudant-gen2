...
If a design document is updated, {{site.data.keyword.cloudant_short_notm}} deletes the indexes from the previous version, and re-creates the index from scratch. If you need to change a design document for a larger database, have a look at the [Design document management guide](/docs/cloudant-gen2?topic=cloudant-gen2-design-document-management#managing-changes-to-a-design-document).
{: note}
...
An important use of design documents is for creating views. For more information about creating views, see [Views (MapReduce)](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce).
...
-	[Search](/docs/cloudant-gen2?topic=cloudant-gen2-cloudant-search)
-	[MapReduce](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce)
...
2.   You created a [search index](/docs/cloudant-gen2?topic=cloudant-gen2-cloudant-search) within the document where you
   [updated](/docs/cloudant-gen2?topic=cloudant-gen2-update-docs) the document with the appropriate field or [created](/docs/cloudant-gen2?topic=cloudant-gen2-create-document) a new document that includes the search index.
...
the [changes feed](/docs/cloudant-gen2?topic=cloudant-gen2-get-changes).
...
Filter functions can also be used to modify a [replication task](/docs/cloudant-gen2?topic=cloudant-gen2-advanced-replication#filtered-replication-adv-repl).
{: tip}
...
:   Accepts only changes for documents that match a specified selector that is defined by using the same [selector syntax](/docs/apis/cloudant/cloudant-gen2#postfind){: external} as described in the Request section, which is used for [`_find`](/docs/apis/cloudant/cloudant-gen2#getdatabaseinformation){: external}.

[`_view`](#the-view-filter)
:   With this function, you can use an existing [map function](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce#a-simple-view) as the filter.
...
The `_selector` filter accepts only changes for documents that match a specified selector, which is defined by using the same [selector syntax](/docs/cloudant-gen2?topic=cloudant-gen2-selector-syntax) that is used
for [`_find`](/docs/apis/cloudant/cloudant-gen2#postfind){: external}.

For more examples that show use of this filter,
see the information on [selector syntax](/docs/cloudant-gen2?topic=cloudant-gen2-selector-syntax).
...
Using the `_view` filter, you can use an existing [map function](/docs/cloudant-gen2?topic=cloudant-gen2-creating-views-mapreduce#a-simple-view) as the filter.
...
| `secObj`  | The [security object](/docs/apis/cloudant/cloudant-gen2#getsecurity){: external} for the database. |
